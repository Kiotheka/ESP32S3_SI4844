...
void lv_spinbox_set_value(lv_obj_t *obj, int64_t v)
{
...
void lv_spinbox_set_digit_format(lv_obj_t *obj, uint32_t digit_count, uint32_t sep_pos)
{
    ...
    if (digit_count < LV_SPINBOX_MAX_DIGIT_COUNT)
    {
        ...
        const int64_t max_val = lv_pow(spinbox->base, digit_count);
...
void lv_spinbox_set_range(lv_obj_t *obj, int64_t range_min, int64_t range_max)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    lv_spinbox_t *spinbox = (lv_spinbox_t *)obj;

    if (range_max > LV_SPINBOX_MAX_RANGE)
        range_max = LV_SPINBOX_MAX_RANGE;
    if (range_min < LV_SPINBOX_MIN_RANGE)
        range_min = LV_SPINBOX_MIN_RANGE;
...
void lv_spinbox_set_cursor_pos(lv_obj_t *obj, uint32_t pos)
{
    ...
    const int64_t step_limit = LV_MAX(spinbox->range_max, LV_ABS(spinbox->range_min));
    const int64_t new_step = lv_pow(spinbox->base, pos);
...
int64_t lv_spinbox_get_value(lv_obj_t *obj)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    lv_spinbox_t *spinbox = (lv_spinbox_t *)obj;
...
void lv_spinbox_step_next(lv_obj_t *obj)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    lv_spinbox_t *spinbox = (lv_spinbox_t *)obj;
    const int32_t new_step = spinbox->step / spinbox->base;
            ...
            if (spinbox->digit_count > 1)
            {
                if (spinbox->digit_step_dir == LV_DIR_RIGHT)
                {
                    ...
                    else
                    {
                        ...
                        spinbox->step = lv_pow(spinbox->base, spinbox->digit_count - 2);
                ...
                else
                {
                    ...
                    if (spinbox->step < lv_pow(spinbox->base, spinbox->digit_count - 1))
                    {
                        lv_spinbox_step_prev(obj);
                    }
                    else
...
static void lv_spinbox_updatevalue(lv_obj_t *obj)
{
    ...
    if (spinbox->base == 16)
    {
        lv_snprintf(digits, LV_SPINBOX_MAX_DIGIT_COUNT_WITH_4BYTES, "%" LV_PRIX32, LV_ABS((uint32_t)spinbox->value));
    }
    else
    {
        lv_snprintf(digits, LV_SPINBOX_MAX_DIGIT_COUNT_WITH_4BYTES, "%" LV_PRId32, LV_ABS((int32_t)spinbox->value));
    }
